...
2. **Repeat Two Steps**  
   1. **Assign Points to Clusters**  
      ...
      - More precisely, we set:  
        $$c^{(i)} = \underset{k \in \{1,\dots,K\}}{\mathrm{argmin}}\;\|\boldsymbol{x}^{(i)} - \boldsymbol{\mu}_k\|^2$$  
...
$$J\bigl(c^{(1)}, \dots, c^{(m)}, \boldsymbol{\mu}_1, \dots, \boldsymbol{\mu}_K\bigr) = \frac{1}{m} \sum_{i=1}^m \bigl\|\boldsymbol{x}^{(i)} - \boldsymbol{\mu}_{c^{(i)}}\bigr\|^2$$
