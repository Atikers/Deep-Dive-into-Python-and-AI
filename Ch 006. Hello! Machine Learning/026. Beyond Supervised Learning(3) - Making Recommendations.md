...
So each movie $i$ is described by a **feature vector** $x^{(i)} = 
\begin{bmatrix}
x_1(i) \\
x_2(i)
\end{bmatrix}$.
...
but we remove the $m^{(j)}$ from the denominator because the reasons are:
...
\sum_{j=1}^{n_u} 
\left[
  \frac{1}{2}\sum_{i : r(i,j)=1} \Bigl(w^{(j)} \cdot x^{(i)} + b^{(j)} - y^{(i,j)}\Bigr)^2
  \;+\; \frac{\lambda}{2}\sum_{k=1}^{n} \Bigl(w_{k}^{(j)}\Bigr)^2
\right].
$$
