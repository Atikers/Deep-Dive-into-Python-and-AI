...
where:
- $W = [w_1 \quad w_2 \quad w_3 \quad w_4 \quad ... \quad w_n]$ is a weight vector containing all our coefficients
...
- $b$ is the bias term
- $\hat{y} = [y^{(1)} \quad y^{(2)} \quad ... \quad y^{(m)}]$ is the vector of predicted outputs
