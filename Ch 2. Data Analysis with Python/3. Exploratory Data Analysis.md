...
- **P-value**: P-value is used to **assess the statistical significance** of the results, **not to measure the strength or certainty of the correlation** itself. We can get the P-value using the `stats.pearsonr()` function from the `Scipy.stats` package.
    ...
    >   - **Null Hypothesis (H₀)**: In the courtroom, the default assumption is that the accused person is innocent. This is like the null hypothesis in statistics, where we assume there's no effect or no difference between groups.
    >   - **Alternative Hypothesis (H₁)**: The prosecutor believes the accused person is guilty and presents evidence to prove it. This is like the alternative hypothesis in statistics, where we believe there is an effect or a difference, and we try to provide evidence for it.
    ...
    >   - **Summary:**
    >     - The null hypothesis (H₀) is like assuming someone is innocent.
    >     - The alternative hypothesis (H₁) is like believing someone is guilty.
...
- Example
  ...
  > where:
  >
  > - ![alt text](image-16.png) is the observed frequency of each category ***i***
  > - ![alt text](image-17.png) is the expected frequency of each category ***i***, calculated as :
