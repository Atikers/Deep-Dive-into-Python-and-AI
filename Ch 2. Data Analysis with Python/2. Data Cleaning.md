...
For example, Standardizing **N.Y./Ny/NY/New York** to **NY** or Converting **mpg => L/100km**
...
- Sometimes the wrong data type is assigned to a variable. For example, `price` might be assigned as a string, but it should be a float.
  - Example:  
...
- Normalization ensures a fair comparison between different features, ensuring they have the same impact. It is crucial for computational reasons.
- For example,
...
You can group values together into **bins**. You can converts numeric variables into categorical variables. **Binning** can improve accuracy of predictive models and help in understanding the data distribution.
...
Most statistical models cannot accept objects or strings as input; they require numerical data for model training. Therefore, categorical variables need to be converted into quantitative variables.
