...
Data can have a variety of types. The main types stored in Pandas objects are `object`, `float`, `int`, and `datetime`. The data type names in Pandas are somewhat different from those in native Python. Below is a comparison of these types:
...
Some types are very similar, such as the numeric data types `int` and `float`. The `object` type in Pandas functions similarly to a `string` in Python, with the exception of the name change. The `datetime` type in Pandas is particularly useful for handling time series data(But see the `datetime` module in Python for more general date and time handling).
...
By default, the DataFrame `describe` function skips rows and columns that do not contain numbers. However, it is possible to make the `describe` method work for `object` type columns as well. To enable a summary of all the columns, we can add the argument `(include='all')` inside the `describe` function : `df.describe(include='all')`.

Now, the output shows the summary of all 26 columns, including `object` type attributes. For the `object` type columns, a different set of statistics is evaluated, such as `unique`, `top`, and `freq`.
...
## 4. Accessing Databases with Python(Please refer to ***Ch 5. "Hello! SQL"*** for more detailed information.)
...
DB API is Python standard API for accessing relational databases. It is a standard that allows you to write a single program that works with multiple kinds of relational databases instead of writing a separate program for each one.
...
With your cursor ready, you can start executing commands to interact with the database. This is like instructing the forklift to retrieve certain items from the warehouse shelves.
  ```python
